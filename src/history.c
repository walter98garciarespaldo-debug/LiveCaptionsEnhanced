...
void save_current_history(const char *path){
    ...
    FILE *f = fopen(path, "w");
    if (f == NULL) {
        fprintf(stderr, "Could not open history file for writing: %s\n", path);
        return;
    }
...
char *history_get_all_text(void) {

    GString *str = g_string_new("");
    
    // We only care about the actual text, not the timestamps for 'Copy All'
    for(size_t i=0; i<past_sessions.num_sessions; i++){
        struct history_session *session = &past_sessions.sessions[i];
        for(size_t j=0; j<session->entries_count; j++){
            struct history_entry *entry = &session->entries[j];
            for(size_t k=0; k<entry->tokens_count; k++){
                g_string_append(str, entry->tokens[k].token);
            }
    ...
    for(size_t j=0; j<active_session.entries_count; j++){
        struct history_entry *entry = &active_session.entries[j];
        for(size_t k=0; k<entry->tokens_count; k++){
            g_string_append(str, entry->tokens[k].token);
        }
        g_string_append_c(str, ' ');
