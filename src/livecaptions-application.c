...
static void
livecaptions_application_startup(GApplication *app) {
    G_APPLICATION_CLASS(livecaptions_application_parent_class)->startup(app);
...
static void livecaptions_application_class_init(LiveCaptionsApplicationClass *klass) {
    ...
    */
    ...
    app_class->startup = livecaptions_application_startup;
...
static void
livecaptions_application_clear_history(G_GNUC_UNUSED GSimpleAction *action,
                                       G_GNUC_UNUSED GVariant      *parameter,
                                       gpointer       user_data)
{
    LiveCaptionsApplication *self = LIVECAPTIONS_APPLICATION(user_data);
    if(self->asr != NULL) {
        asr_thread_clear_history(self->asr);
    }
}

static void
livecaptions_application_copy_all(G_GNUC_UNUSED GSimpleAction *action,
                                  G_GNUC_UNUSED GVariant      *parameter,
                                  gpointer       user_data)
{
    LiveCaptionsApplication *self = LIVECAPTIONS_APPLICATION(user_data);
    
    if (!self->window || !self->window->label) return;

    // We copy only what is on screen (the current label text)
    const char *raw_text = gtk_label_get_text(self->window->label);
    if (!raw_text || strlen(raw_text) == 0) return;

    // Collapse multiple newlines into maximum 1
    ...
    char *text = g_string_free(clean, false);
...
static void livecaptions_application_init(LiveCaptionsApplication *self) {
    ...
    g_autoptr(GSimpleAction) clear_hist_action = g_simple_action_new("clear-history", NULL);
    g_signal_connect(clear_hist_action, "activate", G_CALLBACK(livecaptions_application_clear_history), self);
    g_action_map_add_action(G_ACTION_MAP(self), G_ACTION(clear_hist_action));

    g_autoptr(GSimpleAction) copy_all_action = g_simple_action_new("copy-all", NULL);
    g_signal_connect(copy_all_action, "activate", G_CALLBACK(livecaptions_application_copy_all), self);
    g_action_map_add_action(G_ACTION_MAP(self), G_ACTION(copy_all_action));
